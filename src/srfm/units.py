...
def DU_to_col_den(du):
    """Convert a value in Dobson units to molecues m :sup:`-2`.

    Args:
        du (float): Value in Dobson units.

    Returns:
        col_den (float): Value in molecues m :sup:`-2`.
    ...
    if not isinstance(du, (int, float)):
        raise TypeError("Input du must be int or float.")
