...
The module provides :class:`Iasi_L2`, which parses `.nat` Level 2 files and
exposes geophysical profiles, column integrals, and optional FORLI/Brescia
...
class Iasi_L2:
    ...
    The class reads a full-orbit Level 2 file, validates the MPH/IPR/GIADR
    structure, and returns geophysical retrievals and ancillary metadata for
    every pixel that satisfies the optional filters.

    Args:
      l2fil (str): Path to the Level 2 `.nat` file.
      mph_only (bool, optional): When True, stop after reading the Main Product
        Header and leave retrieval lists empty.
      forli (bool, optional): Include the optional FORLI/Brescia CO, HNO3, O3, and
        SO2 products. Defaults to False.
      latlim (tuple[float, float] | None): Latitude limits `[south, north]`.
      lonlim (tuple[float, float] | None): Longitude limits `[west, east]` where
        the first value is interpreted as the western boundary.
      loclim (list[dict[str, float]] | None): Specific locations (lat, lon,
        `msclin`, etc.) to retain. Only matching pixels are returned.
      **kwargs: Additional MDR field/value filters (for example,
        `flg_retcheck=0` keeps only retrievals with good checks).

    Attributes:
      fail (bool): True when a fatal read error occurred.
      errtxt (str | None): Description of the fatal error, if any.
      mph (dict[str, str]): Main Product Header entries.
      grd (dict[str, np.ndarray]): Profile grid definitions read from the GIADR.
      nloc (int): Number of locations retained after filtering.
      fov (list[int]): Field-of-view indices (1-120) for each selected pixel.
      lin (list[int]): Scan line numbers (equivalent to MDR index).
      stp (list[int]): Cross-scan step numbers (1-30).
      pix (list[int]): Pixel numbers within each step (1-4).
      daylin (list[int]): Julian day of each scan line start.
      msclin (list[int]): Milliseconds from the orbit start at each scan line.
      lat (list[float]): Latitudes of each selected profile [-90, 90].
      lon (list[float]): Longitudes [-180, 180].
      sza (list[float]): Solar zenith angle in degrees.
      zen (list[float]): Satellite zenith angle in degrees.
      saa (list[float]): Solar azimuth angle in degrees.
      azi (list[float]): Satellite azimuth angle in degrees.
      tem_a, h2o_a, o3_a (list[np.ndarray]): First-guess temperature, water-vapour,
        and ozone profiles.
      tem, h2o, o3 (list[np.ndarray]): Retrieved temperature, water-vapour,
        and ozone profiles at the supplied grid levels.
      sft, sft_a (list[float]): Retrieved and first-guess surface temperatures.
      h2o_col, o3_col, n2o_col, co_col, ch4_col, co2_col (list[float]): Column
        integrals for key trace gases.
      sfe (list[np.ndarray]): Surface emissivity at the wavelengths listed in
        `grd['surface_emissivity_wavelengths']`.
      ncld, cld, ctt, ctp, cph (list): Cloud metrics such as number of formations,
        fractional cover, top temperature/pressure, and phase.
      sfp (list[float]): Surface pressure per profile.
      A range of quality/flag fields (`flg_*`) as well as optional FORLI/Brescia
      CO/HNO3/O3/SO2 retrieval lists are initialised in `_inivar`. See that method
      for the full set of attributes made available on each instance.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialise the reader and populate retrieval lists.

        Args:
          l2fil (str): Name of the Level 2 `.nat` file.
          mph_only (bool): When True, only read the Main Product Header.
          forli (bool): Include FORLI/Brescia trace-gas products when available.
          latlim (tuple[float, float] | None): Latitude limits `[south, north]`.
          lonlim (tuple[float, float] | None): Longitude limits `[west, east]`.
          loclim (list[dict[str, float]] | None): List of required locations
            (latitude, longitude, `msclin`, etc.).
          **kwargs: Additional Measurement Data Record filters applied per pixel.
        """
        ...
        if loclim:
            locmsc = []
            for loc in loclim:
                if loc["msclin"] not in locmsc:
                    locmsc.append(loc["msclin"])
        ...
        with open(l2fil, "rb") as f:  # open binary file, readonly
            ...
            for imdr in range(total_mdr):  # Loop over scan lines
                ...
                if latlim:
                    usefov &= self._latlim(mdr, latlim)
                if lonlim:
                    usefov &= self._lonlim(mdr, lonlim)
                if loclim:
                    usefov &= self._loclim(mdr, loclim, locmsc)
                if not any(usefov):
                    continue  # nothing with lat,lon, so skip to next MDR
                ...
                forli = self._read_forli(
                    f, self.NL_HNO3, self.NEVE_HNO3, self.NEVA_HNO3
                )
    ...
    def _check_mph(self):
        ...
        if self.mph["PROCESSOR_MAJOR_VERSION"] != "6":
            self.fail = True
            self.errtxt = (
                "F-Iasi_L2_Class: Expected PROCESSOR_MAJOR_VERSION=6"
                + " but found ="
                + self.mph["PROCESSOR_MAJOR_VERSION"]
            )
        ...
        # Expect 1 Global Internal Auxiliary Data Record: Pressure levels etc
            ...
            self.fail = True
            self.errtxt = (
                "F-Iasi_L2_Class: Expected TOTAL_GIADR=1"
                + " but found ="
                + self.mph["TOTAL_GIADR"]
            )
    ...
    def _get_grd(self, giadr, forli):
        ...
        grd = {
            "nlt": giadr["num_pressure_levels_temp"],
            ...
            "wno_s": 1.0e8 / giadr["surface_emissivity_wavelengths"],
        }  # cm-1
        if forli:
            grd = {
                **grd,
                ...
                "hgt_so2": giadr["brescia_altitudes_so2"] * 1.0,
            }  # m
    ...
    def _get_mdroff(self, f, offset, nmdr):
        ...
        for imdr in range(nmdr):
            ...
            if grh["instrument_group"] == 15:  # standard MDR, 15=IASI L2
                ...
                self.fail = True
                self.errtxt = (
                    "F-Iasi_L2_Class: Unexpected MDR.INSTRUMENT_GROUP ="
                    + str(grh["instrument_group"])
                    + ", expected: 15 or 13"
                )
                return None
    ...
    def _inivar(self, forli):
        ...
        self.flg_cldfrm = (
            []
        )  # (byte) Origin of characterisation of the cloud formations
        ...
        self.flg_fgcheck = (
            []
        )  # (i2)   Check geophys.params from the first guess within bounds
        self.flg_iasibad = []  # (byte) Availability and quality of IASI L1 measurements
        self.flg_initia = (
            []
        )  # (byte) Indicates measurements used in the first guess retrieval
        ...
        self.flg_physcheck = (
            []
        )  # (byte) Potential correct. for superadiabatic/supersat. conditions
        self.flg_retcheck = (
            []
        )  # (i2)   Check that geophys.params from the OEM within bounds
        ...
        if forli:
            ...
            self.so2_col_at_altitudes = (
                []
            )  # SO2 column for plume at diff.est. altitudes [DU]
    ...
    def _read_err(self, f, skip=True):
        ...
        if nerr > 0:
            err["temperature_error"] = np.fromfile(
                f, dtype=">u4", count=nerr * self.NERRT
            )  #
            err["water_vapour_error"] = np.fromfile(
                f, dtype=">u4", count=nerr * self.NERRW
            )  #
    ...
    def _read_giadr(self, f, offset):
        ...
        GIADR = np.dtype(
            [
                ...
                ("brescia_altitudes_so2", ">u2", self.NL_SO2),
            ]
        )  #  10b  1457
    ...
    def _read_ipr(self, f):
        ...
        IPR = np.dtype(
            [
                ...
                (
                    "target_instrument_group",
                    "b",
                ),  #  1b  22b  (15=IASI_L2, 13=Missing data)
        ...
        for i in range(nipr):
            ...
            if ipr["target_record_class"] == 5:
                ...
                if ipr_mdr:
                    print(
                        "W-Iasi_L2_Class: multiple MDR IPR records, only taking first"
                    )
                else:
                ...
                self.fail = True
                self.errtxt = (
                    "F-Iasi_L2_Class: Unexpected Internal Pointer Record,"
                    + " Class="
    ...
    def _set_loc(self, mdr, imdr, idxfov):
        ...
        self.stp += (
            1 + idxfov // 4
        ).tolist()  #  1, 1, 1, 1, 2, 2, ... scan step# (1:30)
        self.lin += [1 + imdr] * len(
            idxfov
        )  #  1, 2, ...             MDR# within file/orbit
        self.pix += (
            1 + idxfov % 4
        ).tolist()  #  1, 2, 3, 4, 1, 2 ...  FOV# with line (1:120)
        self.fov += (
            1 + idxfov
        ).tolist()  #  1, 2, 3, 4, 5, 6, ... Pixel# within step (1:4)
        self.lat += (
            mdr["earth_location"][idxfov, 0] * 1.0e-4
        ).tolist()  # latitutde in deg
        self.lon += (
            mdr["earth_location"][idxfov, 1] * 1.0e-4
        ).tolist()  # longitude in deg
        self.sza += (
            mdr["angular_relation"][idxfov, 0] * 1.0e-2
        ).tolist()  # Solar.zen. in deg
        self.zen += (
            mdr["angular_relation"][idxfov, 1] * 1.0e-2
        ).tolist()  # Satel.zen. in deg
        self.saa += (
            mdr["angular_relation"][idxfov, 2] * 1.0e-2
        ).tolist()  # Soalr.azi. in deg
        self.azi += (
            mdr["angular_relation"][idxfov, 3] * 1.0e-2
        ).tolist()  # Satel.azi. in deg
        self.daylin += [mdr["grh"]["record_start_time"][0]] * len(
            idxfov
        )  # Jul.Day at start of MDR
        self.msclin += [mdr["grh"]["record_start_time"][1]] * len(
            idxfov
        )  # Millsec at start of MDR
    ...
    def _set_brescia(self, brescia, idxfov):
        ...
        altitude = (
            np.where(balt == self.BADU2, np.nan, balt * 1.0)
        ).tolist()  # already in m
        ...
        bt_difference = (
            np.where(btd == self.BADI2, np.nan, btd * 0.01)
        ).tolist()  # 0.01 for K

        bcaa = np.reshape(brescia["col_at_altitudes"], (-1, self.NFOV))
        col_at_altitudes = []
        for ifov in idxfov:
            col_at_altitudes.append(
                np.where(bcaa[:, ifov] == self.BADU2, np.nan, bcaa[:, ifov] * 0.1)
            )  # 0.1 to conver to DU
