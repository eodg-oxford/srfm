"""Reader for IASI L1c data.

- Name: read_iasi_l1c.py
- Parent package: srfm
- Author: Anu Dudhia
...
class Iasi_L1c:
  ...
  Version
    ...
    11FEB25 AD Modified and revert to name Iasi_L1c. 
    13JAN25 AD Add public method 'subset'. Fix bug with AVHRR data.
  ...
  Usage
    On initialisation this returns a list of c.90000 items containing the 
    locations of the IASI spectra, with various options to subset this list.

    Use method .spectrum to get an individual spectrum.
    ...
    Typically call as 
      from iasi_l1c_class import Iasi_L1c

      l1c = Iasi_L1c ( filename, [subset options] )
      if l1c.fail:                      # check for errors
        print(l1c.errmsg)
        stop

      for iloc in range(0,10): 
        print(iloc, l1c.loc[iloc]['lat'], l1c.loc[iloc]['lon'])

      rad = l1c.spectrum (iloc,[options])  
     (options for spectrum method are wnolim, bright)
  ...
  mph_only = T/F, default=False
    ...
    saalim = (saamin,saamax), default=None
             min,max solar azimuth angle (0-360 degrees).
    zenlim = (zenmin,zenmax), default=None
             min,max satellite zenith angle (0=sat overhead).
    azilim = (azimin, azimax), defult=None
             min,max satellite azimuth angle 
  ...
  Methods
    ...
    The following methods are 'public' - intended for the user.
      spectrum      : Extract individual spectrum
      subset        : Reduce l1c structure to subset of locations
  ...
  Data
    ...
    A set of lists each of size nloc:                                   _read_loc
      ...
      azi    [flt] : Satellize Azimuth Angle (0:360)
  ...
  def __init__( self, iasifile, chkqal=(True,True,True), avhrr=False,
                latlim=None, lonlim=None, szalim=None, zenlim=None, saalim=None,
                azilim=None, cldlim=None, lndlim=None, mph_only=False ):
    ...
    Parameters
      ...
      saalim  (flt): min,max solar azimuth angle (0,360) for inclusion
      azilim  (flt): min,max azimut angle (0,360) for inclusion
    ...
    Returns
      obj  containing L1C location data
    ...
    with open ( iasifile, 'rb' ) as f: # Open binary file, read-only
      ...
      if version is None:
        ...
        self.fail = True
        self.errtxt = 'Measurement Data Records are not v5, instead=v'+str(version)
        return
      self.mdruse = mdruse  # Save since this may be useful diagnostic for the user

      self.loc = self._read_loc(f,mdruse,mdroff,avhrr,chkqal,   # Read Location data
                    latlim,lonlim,szalim,zenlim,saalim,azilim,cldlim,lndlim) 
  ...
  def _read_ipr(self,f):
    ...
    ptr['any_mdr'] = min ( ptr['mdr'], ptr['lost'] ) if 'lost' in ptr \
                     else ptr['mdr']
  ...
  def _read_sca(self,f,offset):
    ...
    if sca['IDefScaleSondNbScale'] != 5: 
      self.fail = True
      self.errtxt = 'F-Iasi_L1c: No.band scale factors not set = 5, but =' + \
                    str( sca['IDefScaleSondNbScale'] )
      return None
    ...
    nomlist = (2581,5921,9009,9541,10721,11042) # 645,1480,2242,2385,2680,2760.25
    ioff = nomlist[0]
    for ibnd in range(5):
      ...
      if ista != istanom or iend != iendnom:
        self.fail = True
        self.errtxt = 'F-Iasi_L1c: Band#' + str(ibnd) \
                      + ' scale factor not expected range ' \
                      + str(istanom) + ':' + str(iendnom) + ' but ' \
                      + str(ista)    + ':' + str(iend) 
        return None
      scale[ista-ioff:iend+1-ioff] = 10.0**(-iexp)  # for rad in (W/(m2.sr.m-1))
    return scale
  ...
  def _read_loc(self,f,mdruse,mdroff,avhrr,chkqal,
               latlim,lonlim,szalim,zenlim,saalim,azilim,cldlim,lndlim):
        ...
        for istp in range(self.NSTP):
          for ipix in range(self.NPIX):
            ...
            if lat < -100.0:
              nbadgeo += 1
              continue
            if self._outside(lat,latlim) or \
               self._outside(lon,lonlim) or \
               self._outside(sza,szalim) or \
               self._outside(zen,zenlim) or \
               self._outside(saa,saalim) or \
               self._outside(azi,azilim) or \
               self._outside(cld,cldlim) or \
               self._outside(lnd,lndlim): continue
            # At this point the location will be included 
            if day != dayprv:  # convert day from uint16 to int before adding to jday0
              ymd = int( datetime.fromordinal(int(day)+jday0).strftime('%Y%m%d') )
            ihr = int( msc // 3600000 )
            imin = int ( msc % 3600000 ) // 60000
            isec = int ( msc % 60000 ) // 1000 
            hms = ihr * 10000 + imin * 100 + isec

            locdic = { 'iof':iof,     # Byte Offset for start of spectrum  
                    ...
                    'msclin':mscmdr,  # Millisecs at start of MDR/Scan line
                       ...
                       'lnd':lnd }    # Land fraction [%] 

            if avhrr:
              ncl = mdr_b['GCcsRadAnalNbClass'][istp,ipix]
              pct = 100.0 * mdr_b['GCcsRadAnalWgt'][istp,ipix,:]['i4man'] \
                               * 0.1**mdr_b['GCcsRadAnalWgt'][istp,ipix,:]['ibexp']  
              rav = mdr_b['GCcsRadAnalMean'][istp,ipix,:,:]['i4man'] \
                       * 0.1**mdr_b['GCcsRadAnalMean'][istp,ipix,:,:]['ibexp']
              rsd = mdr_b['GCcsRadAnalStd'][istp,ipix,:,:]['i4man'] \
                        * 0.1**mdr_b['GCcsRadAnalStd'][istp,ipix,:,:]['ibexp']
              rav[:,3:6] *= self.RADFAC
              rsd[:,3:6] *= self.RADFAC
              locdic.update( { 'ncl':ncl, 'pct':pct, 'rav':rav, 'rsd':rsd } )

            loc.append(locdic)
    
    self.stats = {'tot_mdr':nmdr,                  # Tot.No of Meas.Data records
                  ...
                  'geo_loc':nbadgeo }              # No.excluded for bad geo. data
    return loc 
  ...
  def spectrum(self,iloc,wnolim=(645,2760),bright=False):
    ...
    with open ( self.file, 'rb' ) as f: # Open binary file, read-only
      ...
      f.seek(iof+2*i)
      rad = np.fromfile(f,dtype='>i2',count=n) * self.scale[i:j+1] * self.RADFAC
      if bright:
