...
!     Author: R Grainger 1990, 2024
      ...
      Subroutine mie_ewp(Dx, Dxinp, SCm, Inp, Dqv, Dqxt, Dqsc, Pf, Error)
...
!     INPUT
      integer(4), intent(in) :: Dxinp !number of particle size parameters (radii)
      real(8), intent(in) :: Dx(Dxinp) !particle size parameters, >0
      complex(8), intent(in) :: SCm !complex refractive index of the particle
      integer(4), intent(in) :: Inp !number of input scattering angle cosines
      real(8), intent(in) :: Dqv(Inp) !cosines of the scattering angles at which to calculate the intensity functions



!     OUTPUT
      real(8), intent(out) :: Dqxt(Dxinp), Dqsc(Dxinp) !extinction efficiency and scattering efficiency, same size as Dx
      real(8), intent(out) :: PF(Dxinp,Inp) !particle phase function, size Inp
...
!     ACCELERATOR VARIABLES
      ...
      Do J = 1,Dxinp
          If ((Dx(J).Gt.Imaxx)) Then
                Error = 1
                Return
              EndIf
      End Do
       
      Do concurrent (J = 1:Dxinp)

          Cm = SCm
          Ir = 1 / Cm

          Y =  Dx(J) * Cm
          If (Dx(J).Lt.0.02) Then
             NStop = 2
          Else
             If (Dx(J).Le.8.0) Then
                NStop = Dx(J) + 4.00*Dx(J)**(1./3.) + 1.0
             Else
                If (Dx(J).Lt. 4200.0) Then
                   NStop = Dx(J) + 4.05*Dx(J)**(1./3.) + 2.0
                Else
                   NStop = Dx(J) + 4.00*Dx(J)**(1./3.) + 2.0
                End If
             End If 
          End If
...
!          End If
          ...
          Do N = 1,Nstop
             ...
             Turbo = (DN+1D0) / DN
             Rnx = DN/Dx(J)
             Psi = Dble(Tnm1)*Psi1/Dx(J) - Psi0
             APsi = Psi
             Chi = Tnm1*Chi1/Dx(J)       - Chi0
             Xi = Dcmplx(APsi,Chi)
             A = ((D(N)*Ir+Rnx)*APsi-APsi1) / ((D(N)*Ir+Rnx)*  Xi-  Xi1)
             B = ((D(N)*Cm+Rnx)*APsi-APsi1) / ((D(N)*Cm+Rnx)*  Xi-  Xi1)
             Dqxt(J)   = Tnp1 *      Dble(A + B)          + Dqxt(J)
             Dqsc(J)   = Tnp1 * (A*Conjg(A) + B*Conjg(B)) + Dqsc(J)
             Anm1 = A
             Bnm1 = B
             APB = A2 * (A + B)
             AMB = A2 * (A - B)
          ...
          Do concurrent (I = 1:Inp)
             Xs1 = 0.5 * (Sp(I)+Sm(I)) 
             Xs2 = 0.5 * (Sp(I)-Sm(I)) 
             PF(J,I) =  AA * Dble( Xs1*Conjg(Xs1) +Xs2*Conjg(Xs2))
          End Do
