...
INTEGER  NUMU_O
LOGICAL  DEBUG
real(kind=8) :: ACCUR, ALBEDO, BTEMP, FBEAM, FISOT, &
                PHI0, TEMIS, TTEMP, WVNMLO, WVNMHI, UMU0 
real(kind=8),parameter :: EARTH_RADIUS = 6371.0     
DATA PRNT  / .TRUE., 3*.FALSE., .TRUE. /

INTEGER       BRDF_TYPE
DOUBLE PRECISION          BRDF_ARG(4)
LOGICAL       DO_SHADOW
DOUBLE PRECISION          WIND_SPD, REFRAC_INDEX
DOUBLE PRECISION          B0, HH, W
DOUBLE PRECISION          K_VOL, K_ISO, K_GEO
DOUBLE PRECISION          RHO_0, KAPPA, G, H0
DOUBLE PRECISION          FLUX_UP, DFDTAU
INTEGER       NMUG
DOUBLE PRECISION          BDREF
EXTERNAL      BDREF

real(kind=8),dimension(:),allocatable     :: DTAUC, PHI, SSALB, TEMPER, UMU, UTAU                             
real(kind=8),dimension(:,:),allocatable   :: PMOM          
real(kind=8),dimension(:,:,:),allocatable :: RHOQ, RHOU 
real(kind=8),dimension(:),allocatable     :: EMUST, BEMST   
real(kind=8),dimension(:,:),allocatable   :: RHO_ACCURATE                             
real(kind=8),dimension(:),allocatable     :: RFLDIR, RFLDN, FLUP, DFDT, UAVG, ALBMED, TRNMED
real(kind=8),dimension(:,:,:),allocatable :: UU
real(kind=8),dimension(:),allocatable     :: H_LYR
...
PARAMETER  ( MXPROB = 16, MXCASE = 8, MXTAU = 5,  &
             MXMU = 90, MXPHI = 5 )
DOUBLE PRECISION  TSTFIR( MXTAU, MXCASE, MXPROB ), &
...
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE     :: CMPFIR, CMPFDN, CMPFUP, CMPDFD
DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE :: CMPUU 
...
! c **********************************************************************
  ...
  DO 30  ICAS = 1, 2
    ...
    IF ( ICAS.EQ.1 ) THEN
      ...
      CALL  GETMOM( 3, 0.75D0, NMOM, PMOM )
      DTAUC( 1 ) = UTAU( 2 ); SSALB( 1 ) = 1.0
      FBEAM      = PI / UMU0; FISOT      = 0.0
...
!c **********************************************************************
  ...
  DO 70 ICAS = 1, 5
    ...
    IF ( ICAS.EQ.1 ) THEN
      ...
      PHI( 1 )  = 0.0; 
      UTAU( 1 ) = 0.0;  UTAU( 2 ) = 1.0; 
      CALL  GETMOM( 3, 0.05D0, NMOM, PMOM )
...
! c **********************************************************************
  ...
  DO 80 ICAS = 1, 3
    ...
    IF ( ICAS.EQ.1 ) THEN
      ...
      CALL  GETMOM( 1, 0.0D0, NMOM, PMOM(:,1) )
      CALL  GETMOM( 1, 0.0D0, NMOM, PMOM(:,2) )
...
! c **********************************************************************
  ...
  DO 90 ICAS = 1, 3     
    ...
    IF ( ICAS.EQ.1 ) THEN
      ...
      DO LC = 1, NLYR
        CALL  GETMOM( 1, 0.0D0, NMOM, PMOM(:,LC) )
      ENDDO       
      DO LC = 1, NLYR
        DTAUC( LC ) = LC
    ...
    ELSEIF ( ICAS .EQ. 3 ) THEN
      ...
      UTAU( 1 ) = 0.0;  UTAU( 2 ) = 1.05; UTAU( 3 ) = 2.1; UTAU( 4 ) = 6.0; UTAU( 5 ) = 21.0; 
      DO LC = 1, NLYR 
        CALL  GETMOM( 3, DBLE(LC)/7.0D0, NMOM, PMOM(0,LC) )
      ENDDO 
      DO LC = 1, NLYR
        DTAUC( LC ) = LC
        SSALB( LC ) = 0.6 + LC*0.05
  ...
  DO 100 ICAS  =  1, 2
    ...
    IF ( ICAS .EQ. 1 ) THEN
      ...
      DO LC = 1, NLYR
        DTAUC( LC ) = REAL( LC );
        SSALB( LC ) = 0.6 + LC*0.05
        CALL  GETMOM( 3, DBLE(LC)/DBLE(NLYR+1), NMOM, PMOM(0,LC) )
        TEMPER( LC ) = 600.0 + LC*10.0
      ENDDO
      FBEAM      = PI;   FISOT      = 1.0
      PLANK      = .TRUE.; BTEMP      = 700.0; TTEMP  =  550.0; TEMIS      = 1.0
  ...
  DO 120 ICAS = 1, 2
    ...
    ELSEIF( ICAS .EQ. 2 ) THEN
      ...
      DO LC = 1, NLYR
        DTAUC( LC ) = UTAU(LC+1) - UTAU(LC)
        SSALB( LC ) = 0.5
        CALL  GETMOM( 3, 0.9D0, NMOM, PMOM(0,LC) )
      ENDDO
      FBEAM      = 1.0;   FISOT      = 0.0
      PLANK      = .FALSE.;      
      LAMBER     = .TRUE.; ALBEDO   = 1.0; 
...
! c **********************************************************************
  ...
  DO 130 ICAS = 1, 4     
    ...
    ELSEIF( ICAS.EQ.3 ) THEN     
      ...
      DO  LC = 1, NLYR
        DTAUC( LC ) = 1.0 / NLYR
        CALL  GETMOM( 3, 0.8D0, NMOM, PMOM(0,LC) )
      ENDDO
      SSALB( 1 ) = 0.99
      SSALB( 2 ) = 0.50
      PRNT( 4 )  = .TRUE.
...
! c 1. Hapke BRDF   *********************************************
      ...
      CALL  GETMOM( 2, 0.0D0, NMOM, PMOM(:,1) )
      CALL  GETMOM( 6, 0.0D0, NMOM, PMOM(:,2) )
