...
c     .. Scalar Arguments ..
      ...
      LOGICAL   DEBUG
      INTEGER   NSTR, NUMU
      DOUBLE PRECISION      ALBEDO, FBEAM, UMU0
      INTEGER   BRDF_TYPE, NPHI 
      DOUBLE PRECISION      PHI0, PHI(NPHI)
      INTEGER   NMUG

c     ..
c     .. Array Arguments ..
      DOUBLE PRECISION      UMU(NUMU)
      DOUBLE PRECISION      BRDF_ARG(4)

c     ..
c     .. Local Scalars ..
      INTEGER   IQ, IU, J, K, MAZIM, NAZ, NN
      DOUBLE PRECISION      DELM0, PI

c     ..
c     .. Local Arrays ..
      DOUBLE PRECISION      BDR(NSTR/2, 0:NSTR/2), BEM(NSTR/2),
     &          CMU(NSTR), CWT(NSTR), EMU(NUMU),
     &          RMU(NUMU, 0:NSTR/2) 
      DOUBLE PRECISION      RHOQ(NSTR/2, 0:NSTR/2, 0:NSTR-1)
      DOUBLE PRECISION      RHOU(NSTR, 0:NSTR/2, 0:NSTR-1),
     &          EMUST(NUMU), BEMST(NSTR/2)
      DOUBLE PRECISION      BDR_BEAM_ANALYTIC(NUMU,NPHI), DPHI
      DOUBLE PRECISION      GMU(NMUG), GWT(NMUG), COSMP(0:NSTR-1,NMUG/2) 
...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      ALBEDO, DELM0, FBEAM, PI, UMU0
      INTEGER   BRDF_TYPE

c     ..
c     .. Array Arguments ..
      DOUBLE PRECISION      BDR( NN, 0:NN ), BEM( NN ), CMU( * ), 
     &          EMU( NUMU ), RMU( NUMU, 0:NN ), UMU( * ), 
...
c     .. Local Scalars ..
      ...
      DOUBLE PRECISION      DREF, SUM

c     ..
c     .. Local Arrays ..
      DOUBLE PRECISION      GMU( NMUG ), GWT( NMUG ),
     &                      COSMP(0:NSTR-1, NMUG/2)
