...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      ACCUR, ALBEDO, BTEMP, FBEAM, FISOT, PHI0, 
     &          TEMIS, TTEMP, UMU0, WVNMHI, WVNMLO

c     ..
c     .. Array Arguments ..
      LOGICAL   PRNT( 5 )
      DOUBLE PRECISION      ALBMED( MAXUMU ), DFDT( MAXULV ), 
     &          DTAUC( MAXCLY ), FLUP( MAXULV ), PHI( MAXPHI ), 
...
c     .. Local Scalars ..
     ...
     &          NCUT, NN, MCACHE
      DOUBLE PRECISION      ANGCOS, AZERR, AZTERM, BPLANK, COSPHI, 
     &          DELM0, DITHER, DUM, PI, RPD, SGN, TPLANK

c     ..
c     .. Local Arrays ..
      LOGICAL   PRNTU0( 2 )
      INTEGER   IPVT(MAXCMU*MAXCLY ), LAYRU( MAXULV )
      DOUBLE PRECISION      AMB(MAXCMU/2,MAXCMU/2),  
     &          APB(MAXCMU/2,MAXCMU/2), ARRAY(MAXCMU,MAXCMU),
     ...
     &          ZBEAM( MAXUMU, MAXCLY )
      DOUBLE PRECISION      ZJ( MAXCMU), ZPLK0( MAXCMU, MAXCLY),
     ...
     &          EVECCD(MAXCMU/2,MAXCMU/2), WKD( MAXCMU )
      DOUBLE PRECISION      YLM0_CACHE(0:MAXCMU,0:MAXCMU),
     &          YLMC_CACHE(0:MAXCMU,MAXCMU,0:MAXCMU),
     &          YLMU_CACHE(0:MAXCMU,MAXUMU,0:MAXCMU)
      LOGICAL   YLM0_READY(0:MAXCMU), YLMC_READY(0:MAXCMU),
     &          YLMU_READY(0:MAXCMU)

c     ..
c     .. Version 3 .. 
      DOUBLE PRECISION      RHOQ(MAXCMU/2, 0:MAXCMU/2, 0:(MAXCMU-1)), 
     &          RHOU(MAXUMU,   0:MAXCMU/2, 0:(MAXCMU-1)),
     &          EMUST(MAXUMU), BEMST(MAXCMU/2)
      DOUBLE PRECISION      UMU0DI, UMU0SQ, DENOM
      DOUBLE PRECISION      RHO_ACCURATE(MAXUMU,MAXPHI)  
      DOUBLE PRECISION      EIGEN_MAT(MAXCMU/2, MAXCMU/2 )  

c     .. Version 3: spherical correction ..
      DOUBLE PRECISION      EARTH_RADIUS, H_LYR(0:MAXCLY)
      DOUBLE PRECISION      UMU0L(MAXCLY)
...
c     .. External Functions ..
      DOUBLE PRECISION      PLKAVG, D1MACH, RATIO
...
!f2py intent(in, out) RFLDIR, RFLDN, FLUP, DFDT, UAVG, UU, ALBMED, TRNMED
...
c     ** Handle special case for getting albedo and transmissivity of 
c     ** medium for many beam angles at once.
      IF( IBCND.EQ.1 ) THEN
...
c     ** Precompute Legendre tables for all azimuth orders once.
      DO 45 MCACHE = 0, NAZ

        IF( FBEAM.GT.0.0 ) THEN
          ANGCOS = -UMU0
          CALL LEPOLY0( MCACHE, MAXCMU, NSTR-1, ANGCOS, SQT, YLM0 )
          DO 1035 L = MCACHE, NSTR - 1
            YLM0_CACHE( L, MCACHE ) = YLM0( L, 1 )
 1035     CONTINUE
          YLM0_READY( MCACHE ) = .TRUE.
        ENDIF

        IF( .NOT.ONLYFL .AND. USRANG ) THEN
          CALL LEPOLY( NUMU, MCACHE, MAXCMU, NSTR-1, UMU, SQT, YLMU )
          DO 1037 L = MCACHE, NSTR - 1
            DO 1036 IU = 1, NUMU
              YLMU_CACHE( L, IU, MCACHE ) = YLMU( L, IU )
 1036       CONTINUE
 1037     CONTINUE
          YLMU_READY( MCACHE ) = .TRUE.
        ENDIF

        CALL LEPOLY( NN, MCACHE, MAXCMU, NSTR-1, CMU, SQT, YLMC )
        SGN  = -1.0
        DO 1040 L = MCACHE, NSTR - 1
          SGN  = -SGN
          DO 1038 IQ = NN + 1, NSTR
            YLMC( L, IQ ) = SGN*YLMC( L, IQ - NN )
 1038     CONTINUE
 1040   CONTINUE

        DO 1044 L = MCACHE, NSTR - 1
          DO 1042 IQ = 1, NSTR
            YLMC_CACHE( L, IQ, MCACHE ) = YLMC( L, IQ )
 1042     CONTINUE
 1044   CONTINUE
        YLMC_READY( MCACHE ) = .TRUE.
...
c       ** (b) computational and user polar angle cosines
        ...
        IF( FBEAM.GT.0.0 ) THEN
          DO 2060 L = MAZIM, NSTR - 1
            YLM0( L, 1 ) = YLM0_CACHE( L, MAZIM )
 2060     CONTINUE
        ENDIF


        IF( .NOT.ONLYFL .AND. USRANG ) THEN
          IF( .NOT. YLMU_READY( MAZIM ) )
     &      CALL ERRMSG( 'Legendre cache missing for user angles', 
     &                    .FALSE. )
          DO 2070 L = MAZIM, NSTR - 1
            DO 2065 IU = 1, NUMU
              YLMU( L, IU ) = YLMU_CACHE( L, IU, MAZIM )
 2065       CONTINUE
 2070     CONTINUE
        ENDIF

        IF( .NOT. YLMC_READY( MAZIM ) )
     &      CALL ERRMSG( 'Legendre cache missing for quadrature angles',
     &                    .FALSE. )
        DO 2090 L = MAZIM, NSTR - 1
          DO 2080 IQ = 1, NSTR
            YLMC( L, IQ ) = YLMC_CACHE( L, IQ, MAZIM )
 2080     CONTINUE
 2090   CONTINUE
...
c         ** Solve eigenfunction problem in Eq. STWJ(8B), STWL(23f); 
...
c         ** Version 3: upgraded subroutine UPBEAM to use reduced order 
c         ** matrix and LAPACK.
          IF( FBEAM.GT.0.0 ) THEN
...
c       ** Set coefficient matrix of equations combining boundary and 
c       ** layer interface conditions.
...
c           ** Save azimuthally averaged intensities at quadrature 
c           ** angles
            DO 100 LU = 1, NTAU
              DO 90 IQ = 1, NSTR
                U0U( IQ, LU ) = U0C( IQ, LU )
...
c         ** Print azimuthally averaged intensities at user angles
  ...
  140       CONTINUE
        ...
        ELSE
c         ** Increment intensity by current azimuthal component (Fourier 
...
c     ** Print intensities
     ...
     &               MAXUMU )
      ...
      IF( PASS1 ) THEN
c       ** Compare test case results with correct answers and abort if 
c       ** bad
         COMPAR = .TRUE.
         CALL SLFTST( CORINT, ACCUR, ALBEDO, BTEMP, DELTAM, DTAUC( 1 ),
     &                FBEAM, FISOT, IBCND, LAMBER, NLYR, PLANK, NPHI,
...
c     .. Array Arguments ..
      ...
      DOUBLE PRECISION      AA( IA, M ), EVAL( M ), EVEC( IEVEC, M )
...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      FBEAM
      DOUBLE PRECISION      UMU0L(*), DTAUCP( * )
c     ..
c     .. Array Arguments ..

      INTEGER   LAYRU( * )
      DOUBLE PRECISION      GC( MXCMU, MXCMU, * ), KK( MXCMU, * ), 
     &          LL( MXCMU, * ), TAUCPR( 0:* ), UTAUPR( MXULV ), 
     &          UUM( MXUMU, MXULV ), ZPLK0( MXCMU, * ), 
     &          ZPLK1( MXCMU, * ), ZZ( MXCMU, * )
c     ..
c     .. Local Scalars ..

      INTEGER   IQ, JQ, LU, LYU, LC
      DOUBLE PRECISION      BEAMFAC, ZINT
c     ..
c     .. Intrinsic Functions ..
...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      FBEAM, PI, UMU0, UMU0L( * )
c     ..
c     .. Array Arguments ..

      LOGICAL   PRNT( * )
      INTEGER   LAYRU( MXULV )
      DOUBLE PRECISION      CMU( MXCMU ), CWT( MXCMU ), DFDT( MAXULV ),
...
c     .. Local Scalars ..
      ...
      INTEGER   IQ, JQ, LU, LYU, LC
      DOUBLE PRECISION      ANG1, ANG2, DELTA1, DELTA2, DIRINT, FACT,
     &                      FDNTOT, FNET
      DOUBLE PRECISION      PLSORC, ZINT
      DOUBLE PRECISION      FACT2, PI_INV, TWO_PI, FOUR_PI
c     ..
c     .. External Subroutines ..
...
c  condiser pseudo spherical correction            
   ...
   80 CONTINUE
     ...
     &        '     Angle (deg)   cos(Angle)     Intensity'
            ...
            DO 90 IQ = 1, NN
               ANG1 = ( 180.D0 * PI_INV )*ACOS( CMU( 2 *NN-IQ+1 ) )
               ANG2 = ( 180.D0 * PI_INV )*ACOS( CMU( IQ ) )
...
c     .. Scalar Arguments ..
     ...
     &          NUMU
      DOUBLE PRECISION      DITHER, FBEAM, PI, RPD, UMU0
c     ..
c     .. Array Arguments ..

      INTEGER   LAYRU( * )
      DOUBLE PRECISION      DTAUC( * ), FLYR( * ), OPRIM( * ), 
     &          PHASA( * ), PHAST( * ), PHASM( * ), PHIRAD( * ),
...
c     .. Local Scalars ..
      ...
      DOUBLE PRECISION      CTHETA, DTHETA, DUIMS, PL, PLM1, PLM2, 
     &          THETA0, THETAP, USSNDM, USSP, SQRT1MU0, SQRT1MUI
      DOUBLE PRECISION      f, sigma_sq, c 
c     ..
c     .. External Functions ..

      DOUBLE PRECISION      SECSCA, SINSCA
...
c                                ** Start loop over zenith angles
      ...
      DO 110 IU = 1, NUMU
         ...
         SQRT1MUI = SQRT( 1.D0 - UMU( IU )*UMU( IU ) )
...
c                     (DISORT3 paper Eq. (43), third term on the righ 
c                     hand side)
c          
...
c +-------------------------------------------------------------------+
      ...
      DOUBLE PRECISION      RHO_APPROX(NUMU,NPHI)
      DOUBLE PRECISION      RHO_ACCURATE(MAXUMU, MAXPHI)
      LOGICAL   LYRCUT
      INTEGER   NUMU, NPHI, MAXUMU, MAXPHI, MXUMU, MI, NAZZ, NSTR
      DOUBLE PRECISION      UMU(MAXUMU), PHI(MAXPHI), PHI0, FBEAM, UMU0
      INTEGER   IU, J, MAXULV, LAYRU( * )
      DOUBLE PRECISION      TAUCPR( 0:* ), UTAUPR( * )
      DOUBLE PRECISION      RHOU(MXUMU,0:MI, 0:NAZZ)
      DOUBLE PRECISION      DRHO, USS, PI, UU( MAXUMU, MAXULV, * )
      DOUBLE PRECISION      BDR_APPROX, BASE, UMU_INV
...
c ---------------------------------------------------------------------
      DOUBLE PRECISION FUNCTION  SECSCA( CTHETA, FLYR, LAYRU, MAXMOM, 
     &                       NMOM, NSTR, PMOM, SSALB, DTAUC, TAUC, UMU, 
     &                       UMU0, UTAU, FBEAM, PI )
...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      CTHETA, FBEAM, PI, UMU, UMU0, UTAU
c     ..
c     .. Array Arguments ..
      DOUBLE PRECISION      DTAUC( * ), FLYR( * ), PMOM( 0:MAXMOM, * ), 
     &          SSALB( * ), TAUC( 0:* )
c     ..
c     .. Local Scalars ..
      INTEGER   K, LYR
      DOUBLE PRECISION      FBAR, GBAR, PL, PLM1, PLM2, PSPIKE, STAU,
     &          UMU0P, WBAR, ZERO, DENOM, DTAUL, FWBAR, INV_DENOM
c     ..
c     .. External Functions ..
...
c     .. Array Arguments ..
      ...
      INTEGER   LAYRU( * )
      DOUBLE PRECISION      CMU( MXCMU ), CWT( MXCMU ), DTAUC( * ), 
     &          DTAUCP( * ), EXPBEA( 0:* ), FLYR( * ), GL( 0:NSTR, * ), 
     &          OPRIM( * ), PMOM( 0:MAXMOM, * ), SSALB( * ), 
     &          TAUC( 0:* ), TAUCPR( 0:* ), UMU( MAXUMU ), UTAU( * ), 
     &          UTAUPR( * ), SQT(2*NSTR), UMU0

      DOUBLE PRECISION      EARTH_RADIUS, H_LYR(0:NLYR),
     &          UMU0P(NLYR,NLYR), UMU0L(NLYR)
      LOGICAL   DO_PSEUDO_SPHERE
c     ..
c     .. Local Scalars ..

      INTEGER   IQ, IU, K, LC, LU, n
      DOUBLE PRECISION      ABSCUT, ABSTAU, F, YESSCT, TAU_SLANT(0:NLYR)
      DOUBLE PRECISION      sigma_sq, c, LOG_D1, UMU0INV
      INTEGER   NAZZ, MI, NS
c     ..
c     .. External Subroutines ..

      EXTERNAL  ERRMSG, QGAUSN, D1MACH
      DOUBLE PRECISION      D1MACH
c     ..
c     .. Intrinsic Functions ..
...
C        ** if(PMOM(NSTR,LC).gt.PMOM(NSTR+1,LC)) then ! fix to avoid 
c        ** negative sigma_sq
             f = pmom(nstr, lc)
...
!           PRINT*, F, C, sigma_sq
C          else
C            f = PMOM( NSTR, LC ) ! go back to delta-M when 
c     &          PMOM(NSTR,LC) < PMOM(NSTR+1,LC)
C          endif  
...
!                print*, LC, TAUCPR(LC), -TAUCPR( LC )/UMU0
                IF( TAUCPR( LC )*UMU0INV .LT. LOG_D1 ) THEN 
...
C    &          .True. )
       IF( ABS( UMU0-CMU( IQ ) )/UMU0.LT.10.0*D1MACH(4) ) 
     &     UMU0 = UMU0+10.0*D1MACH(4)
...
CC
CCc                               ** Shift positive user angle cosines 
CCc                               ** to upper locations and put
CCc                               **  negatives in lower locations
CC        DO 120 IU = 1, NUMU
...
c        special banded-matrix format required by LAPACK/LINPACK 
c        routines
c
c
c    I N P U T      V A R I A B L E S:
...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      DELM0
c     ..
c     .. Array Arguments ..

      DOUBLE PRECISION      BDR( NN, 0:NN ), CBAND(9*NN-2,NSTR*NLYR), 
     &          CMU( MXCMU ), CWT( MXCMU ), DTAUCP( * ), 
     &          GC( MXCMU, MXCMU, * ), KK( MXCMU, * ), TAUCPR( 0:* ), 
     &          WK( MXCMU )
c     ..
c     .. Local Scalars ..

      INTEGER   IQ, IROW, JCOL, JQ, K, LC, LDA, NCD, NNCOL, NSHIFT
      DOUBLE PRECISION      EXPA, SUM, SCALE
      LOGICAL   NOAZ
c     ..
c     .. External Subroutines ..
...
c     ..
      ...
      NSHIFT = LDA - 2*NSTR + 1
      NCOL   = 0
      NOAZ   = LYRCUT .OR. ( LAMBER .AND. DELM0.EQ.0.D0 )
      SCALE  = 1.D0 + DELM0
...
c ---------------------------------------------------------------------
      DOUBLE PRECISION FUNCTION  SINSCA( DITHER, LAYRU, NLYR, PHASE, 
     &                       OMEGA, TAU, UMU, UMU0, UTAU, FBEAM, PI )
...
c     .. Scalar Arguments ..
      ...
      INTEGER   LAYRU, NLYR
      DOUBLE PRECISION      DITHER, FBEAM, PI, UMU, UMU0, UTAU
c     ..
c     .. Array Arguments ..

      DOUBLE PRECISION      OMEGA( * ), PHASE( * ), TAU( 0:* )
!      real      rho
c     ..
c     .. Local Scalars ..

      INTEGER   LYR
      DOUBLE PRECISION      EXP0, EXP1, UMU_INV, UMU0_INV, UMU_UMU0
...
c-----------------------------------------------------------------------
      DOUBLE PRECISION FUNCTION BDR_APPROX( DPHI, NSTR, PI, RHOU )
c     ** Version 3 function

      DOUBLE PRECISION     DPHI
      INTEGER  NSTR
      INTEGER  M
      DOUBLE PRECISION     RHOU(0:*)
      DOUBLE PRECISION     RHO_APPROX, RHO_FLOURIER(0:NSTR)

      DOUBLE PRECISION     BDREF
      EXTERNAL BDREF

      DOUBLE PRECISION     COSMPHI
      DOUBLE PRECISION     PI
      DOUBLE PRECISION     DPHI_RAD
      INTEGER  NAZ


      NAZ = NSTR-1
      DPHI_RAD = DPHI*PI/180.0D0
...
c     .. Array Arguments ..
      ...
      DOUBLE PRECISION      AMB( NN, NN ), APB( NN, NN ),
     &          ARRAY( NN, NN ), CC(NSTR,NSTR ), CMU( MXCMU ), 
     &          CWT( MXCMU ), EVAL( NN ), EVECC( NSTR,NSTR ), 
     &          GC( MXCMU, MXCMU ), GL( 0:NSTR ), KK( MXCMU ), 
     &          YLMC( 0:MXCMU, MXCMU )
...
c     .. Local Scalars ..
      ...
      DOUBLE PRECISION      ALPHA, BETA, GPMIGM, GPPLGM, SUM

c      .. Local Array ..
      DOUBLE PRECISION      TMP(NN, NN)
...
c
c       B        :  Right-hand side vector of Eq. SC(5) going into
c                   DGBSL; returns as solution vector of Eq. SC(12),
...
c
c   Called by- DISORT
c   Calls- ZEROIT, DGBCO, ERRMSG, DGBSL
c +-------------------------------------------------------------------+
...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      BPLANK, FBEAM, FISOT, PI, TPLANK, UMU0
c     ..
c     .. Array Arguments ..

      INTEGER   IPVT( * )
      DOUBLE PRECISION      B( NSTR*NLYR ), BDR( NN, 0:NN ), BEM( NN ),
...
c     .. Local Scalars ..
      ...
      INTEGER   IPNT, IQ, IT, JQ, LC, NCD, NCOL_NN, INFO, LCOLS,
     &          IROW, JCOL
      DOUBLE PRECISION      BDR_IQ0, BEM_IQ_BPLANK, CWT_CMU, EXPBEA_LC,
     &                      EXPBEA_NCUT, SUM, TAUCPR_LC, TAUCPR_NCUT,
     &                      TOP_PLANK, UMU0_FBEAM_PI
      DOUBLE PRECISION      ZZ_BOTTOM, ZZ_DEL, ZZ_TOP, ZPLK0_DEL,
     &                      ZPLK0_TOP, ZPLK1_DEL
      DOUBLE PRECISION      ZPLK0_IQ_NCUT, ZPLK1_IQ_NCUT, ZZ_IQ_NCUT
      DOUBLE PRECISION      ZPLK0_JQ_NCUT, ZPLK1_JQ_NCUT, ZZ_JQ_NCUT
      DOUBLE PRECISION      MAX_DIFF, CACHE_TOL
c     ..
c     .. LU cache (reused if CBAND unchanged) ..

      LOGICAL   LU_READY
      INTEGER   LAST_NCOL, LAST_NCD, LAST_MAZIM
      DOUBLE PRECISION, ALLOCATABLE, SAVE :: CBAND_LU(:,:),
     &          CBAND_ORIG(:,:)
      INTEGER, ALLOCATABLE, SAVE :: IPVT_CACHE(:)
      SAVE      LU_READY, LAST_NCOL, LAST_NCD, LAST_MAZIM
      DATA      LU_READY /.FALSE./
c     ..
c     .. External Subroutines ..

      EXTERNAL  ERRMSG, DGBCO, DGBSL, ZEROIT
c     ..
c     .. Intrinsic Functions ..
...
c      
c
c                     ** Cache LU factorization to avoid repeating it
c                     ** when CBAND is unchanged.

      IF( ALLOCATED( CBAND_LU ) ) THEN
         IF( ( SIZE( CBAND_LU, 1 ).NE.(9*NN-2) ) .OR.
     &       ( SIZE( CBAND_LU, 2 ).NE.LCOLS ) ) THEN
            DEALLOCATE( CBAND_LU )
            DEALLOCATE( CBAND_ORIG )
            LU_READY = .FALSE.
         END IF
      END IF

      IF( ALLOCATED( IPVT_CACHE ) ) THEN
         IF( SIZE( IPVT_CACHE, 1 ).NE.NCOL ) THEN
            DEALLOCATE( IPVT_CACHE )
            LU_READY = .FALSE.
         END IF
      END IF

      IF( .NOT. ALLOCATED( CBAND_LU ) ) THEN
         ALLOCATE( CBAND_LU( 9*NN-2, LCOLS ) )
         ALLOCATE( CBAND_ORIG( 9*NN-2, LCOLS ) )
         LU_READY = .FALSE.
      END IF

      IF( .NOT. ALLOCATED( IPVT_CACHE ) ) THEN
         ALLOCATE( IPVT_CACHE( NCOL ) )
         LU_READY = .FALSE.
      END IF

      IF( LU_READY .AND. LAST_NCOL.EQ.NCOL .AND.
     &    LAST_NCD.EQ.NCD .AND. LAST_MAZIM.EQ.MAZIM ) THEN
         MAX_DIFF = 0.0D0
         DO 3161 IROW = 1, 9*NN-2
            DO 3162 JCOL = 1, LCOLS
               MAX_DIFF = MAX( MAX_DIFF,
     &                 ABS( CBAND_ORIG( IROW, JCOL ) -
     &                     CBAND( IROW, JCOL ) ) )
 3162       CONTINUE
 3161    CONTINUE
         IF( MAX_DIFF.GT.CACHE_TOL ) LU_READY = .FALSE.
      ELSE
         LU_READY = .FALSE.
      END IF

      IF( .NOT. LU_READY ) THEN
         CBAND_ORIG(1:9*NN-2,1:LCOLS) =
     &       CBAND(1:9*NN-2,1:LCOLS)
         CBAND_LU(1:9*NN-2,1:LCOLS) =
     &       CBAND_ORIG(1:9*NN-2,1:LCOLS)

         CALL DGBTRF( NCOL, NCOL, NCD, NCD, CBAND_LU, 9*NN-2, IPVT,
     &                INFO )
 
         IF( INFO .NE. 0 ) 
     &      CALL ERRMSG('SOLVE0--DGBTRF says matrix near singular',
     &                   .FALSE.)
...
c                   ** decomposed.  Solution is returned in B.
      ...
      IF( LU_READY ) IPVT(1:NCOL) = IPVT_CACHE(1:NCOL)

      CALL DGBTRS( 'N', NCOL, NCD, NCD, 1, CBAND_LU, 9*NN-2, IPVT,
     &              B, NSTR*NLYR, INFO   )
...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      ALBEDO, FBEAM 
!      REAL      WVNMHI, WVNMLO, UMU0

c     ..
c     .. Array Arguments ..
      DOUBLE PRECISION      BDR( NN, 0:NN ), BEM( NN ), EMU( NUMU ),
     &          RMU( NUMU, 0:NN ), UMU( * )
c    ..
      DOUBLE PRECISION RHOQ( MI, 0:MI, 0:NAZZ ), 
     &     RHOU( MXUMU, 0:MI, 0:NAZZ ), EMUST( NUMU ), BEMST( MI )
c    ..
...
c     .. Array Arguments ..
      ...
      DOUBLE PRECISION      CWT( MXCMU ), EVECC( NSTR, NSTR ), 
     &          GL( 0:NSTR ), GU( MXUMU, MXCMU ), WK( MXCMU ), 
     &          YLMC( 0:MXCMU, MXCMU ), YLMU( 0:MXCMU, MXUMU )
c     ..
c     .. Local Scalars ..

      INTEGER   IQ, IU, JQ, L
      DOUBLE PRECISION      CWT_JQ, GLHALF, SUM, YLMC_LJ
c     ..
...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      DELM0, FBEAM, OPRIM, PI, XR0, XR1
c     ..
c     .. Array Arguments ..

      DOUBLE PRECISION      CWT( MXCMU ), GL( 0:NSTR ), PSI0( MXCMU ),
...
c     .. Local Scalars ..
      ...
      INTEGER   IQ, IU, JQ
      DOUBLE PRECISION      CWT_JQ, FACT, GL_IQ, PSUM, PSUM0, PSUM1,
     &                      PSI0_IQ, PSI1_IQ, SUM, SUM0, SUM1, YLMC_IQ,
     &                      YLMU_IQ
      DOUBLE PRECISION      XR0S, XR1S
c     ..
...
c
c   Called by- DISORT
c   Calls- DGECO, ERRMSG, DGESL
c +-------------------------------------------------------------------+

c     .. Scalar Arguments ..

      INTEGER   MAZIM, MXCMU, NN
      DOUBLE PRECISION      DELM0, FBEAM, PI, UMU0
c     ..
c     .. Array Arguments ..
...
c     .. Local Scalars ..
      ...
      INTEGER   IQ, K, KQ, JOB, NTERMS

      DOUBLE PRECISION    LEFT_MAT(NN,NN), ZZP(NN), ZZM(NN)
      DOUBLE PRECISION    SUM, SUM1, SUM2, ZJM(NN), ZJP(NN), FACTOR,
     &                    CMU_INV, UMU0_INV, UMU0_INV2,
     &                    AMB_IQKQ, CMU_IQ, UMU0_LOC, ZJM_IQ, ZZM_KQ
      DOUBLE PRECISION    GLYLM0(0:MXCMU)
      INTEGER   INFO

c     ..
c     .. External Subroutines ..

      EXTERNAL  ERRMSG, DGETRS, DGETRF, DGEMV, DDOT 
c     ..
...
c      LEFT_MAT = ARRAY*UMU0**2
      ...
      FACTOR = ( 2d0-DELM0 ) * FBEAM / ( 4d0*PI )
      NTERMS = 2*NN - MAZIM

      DO 9030 K = MAZIM, 2*NN-1
         GLYLM0( K ) = GL( K ) * YLM0( K, 1 )
 9030 CONTINUE

      DO 50 IQ = 1, NN
         LEFT_MAT(IQ,IQ) = LEFT_MAT(IQ,IQ) - UMU0_INV2

         SUM1 = DDOT( NTERMS, GLYLM0( MAZIM ), 1,
     &                YLMC( MAZIM, IQ ), 1 )
         SUM2 = DDOT( NTERMS, GLYLM0( MAZIM ), 1,
     &                YLMC( MAZIM, IQ+NN ), 1 )
   ...
   50 CONTINUE
      ...
      CALL DGEMV( 'N', NN, NN, 1.0D0, APB, NN, ZJM, 1, 0.0D0, ZZM, 1 )

      DO 70 IQ = 1, NN
c         ZZM(IQ) = -SUM*UMU0**2- ZJP(IQ)*UMU0
         ZZM(IQ) = -ZZM(IQ) - ZJP(IQ)*UMU0_INV
...
c                ** ZJ;  return solution(s) in ZJ
  ...
  100     CONTINUE 
   ...
   90   CONTINUE
        ...
        DO 110 IQ = 1,NN
           ZJ( IQ )     = 0.5*(ZZP(IQ)+ZZM(IQ))
           ZJ( NN+IQ )  = 0.5*(ZZP(IQ)-ZZM(IQ))
           ZZ( IQ+NN )  = ZJ( IQ )
           ZZ( NN+1-IQ) = ZJ( IQ + NN )
  110   CONTINUE
...
c
c   Called by- DISORT
c   Calls- DGETRF, ERRMSG, DGETRS
c +-------------------------------------------------------------------+

c     .. Scalar Arguments ..

      INTEGER   MXCMU, NN, NSTR
      DOUBLE PRECISION      OPRIM, XR0, XR1
c     ..
c     .. Array Arguments ..

      INTEGER   IPVT( * )
      DOUBLE PRECISION      ARRAY(NSTR,NSTR ), CC(NSTR,NSTR ), 
     &          CMU( MXCMU ), WK( MXCMU ), Z0( MXCMU ), Z1( MXCMU ), 
     &          ZPLK0( MXCMU ), ZPLK1( MXCMU )
c     ..
c     .. Local Scalars ..

      INTEGER   IQ, JQ, INFO
      DOUBLE PRECISION      OM1, XR0S, XR1S
c     ..
c     .. External Subroutines ..

      EXTERNAL  ERRMSG, DGETRF, DGETRS
c     ..

c     .. Dummy use to silence unused WK ..

      IF( NSTR.LT.0 ) WK(1) = WK(1)
...
c                       ** except ZJ replaced by Z1 and Z0
      CALL DGETRF( NSTR, NSTR, ARRAY, NSTR, IPVT, INFO )
      IF( INFO .NE. 0 )
     &    CALL ERRMSG('UPISOT--DGETRF matrix near singular',.FALSE.)

      CALL DGETRS( 'N', NSTR, 1, ARRAY, NSTR, IPVT, Z1, NSTR, INFO )

      DO 30 IQ = 1, NSTR
         Z0( IQ ) = XR0S + CMU( IQ ) * Z1( IQ )
   30 CONTINUE

      CALL DGETRS( 'N', NSTR, 1, ARRAY, NSTR, IPVT, Z0, NSTR, INFO )
...
c     .. Scalar Arguments ..
     ...
     &          NUMU
      DOUBLE PRECISION      BPLANK, DELM0, FBEAM, FISOT, PI, TPLANK, 
     &                      UMU0, UMU0L(*)
c     ..
c     .. Array Arguments ..

      INTEGER   LAYRU( * )
      DOUBLE PRECISION      CMU( MXCMU ), CWT( MXCMU ), DTAUCP( * ), 
     &          EMU( NUMU ), EXPBEA( 0:* ), GC( MXCMU, MXCMU, * ),
...
c     .. Local Scalars ..
      ...
      DOUBLE PRECISION      BNDDFU, BNDDIR, BNDINT, DENOM, DFUINT, DTAU,
     &          DTAU1, DTAU2, EXP0, EXP1, EXP2, EXPN, F0N, F1N, FACT,
     &          PALINT, PLKINT, SGN, UMU_INV, UMU_VAL, RMU_IQ, CMU_IQ,
     &          CWT_IQ, KK_IQ
      DOUBLE PRECISION      UMU0L_LYU, UMU0L_LC, EXPBEA_LYUM, EXPBEA_LYU
      DOUBLE PRECISION      GU_IQ, KK_NEG, Z0U_IU, Z1U_IU
      DOUBLE PRECISION      DTAU_LYU, TAUCPR_LCM, TAUCPR_LC
      DOUBLE PRECISION      TAUCPR_LYUM, TAUCPR_LYU, UTAUPR_LU
c     ..
c     .. Intrinsic Functions ..
...
c ---------------------------------------------------------------------
      DOUBLE PRECISION FUNCTION  XIFUNC( UMU1, UMU2, UMU3, TAU )
...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      TAU, UMU1, UMU2, UMU3
c     ..
c     .. Local Scalars ..

      DOUBLE PRECISION      EXP1, UMU1_INV, UMU2_INV, UMU3_INV, X1, X2,
     &                      UMU1UMU2
c     ..
c     .. Intrinsic Functions ..
...
c     .. Array Arguments ..
      ...
      DOUBLE PRECISION      DTAUC( MAXCLY ), PHI( MAXPHI ),
...
c     .. Local Scalars ..
      ...
      DOUBLE PRECISION      FLXALB, RMU, YESSCT
c     ..
c     .. External Functions ..
...
! TOP LAYER IF ADDED 2017-11-27 TO AVOID CHECKING UMU AND OTHER VARS 
c WHEN ONLYFL = .TRUE.  
      IF( .NOT. ONLYFL ) THEN   
      IF( USRANG ) THEN
...
c ---------------------------------------------------------------------
      DOUBLE PRECISION FUNCTION  DREF( MU )
...
c     .. Local Arrays ..
      ...
      DOUBLE PRECISION      GMU( NMUG ), GWT( NMUG )
c     ..
c     .. External Functions ..
...
c     .. Array Arguments ..
      ...
      DOUBLE PRECISION      MU( * ), YLM( 0:MAXMU, * ), SQT( * )
c     ..
c     .. Local Scalars ..

      INTEGER   I, L
      DOUBLE PRECISION      A, INVL, INV_TMP1, MUVAL, TMP1, TMP2
c     ..
...
c     .. Local Scalars ..
      ...
      INTEGER   L
      DOUBLE PRECISION      A, INVL, INV_TMP1, TMP1, TMP2
c     ..
...
c     .. Local Scalars ..
      ...
      DOUBLE PRECISION      C2, CONC, DEL, EPSIL, EX, EXM, HH, MV, 
     &          OLDVAL, PI, SIGDPI, SIGMA, VAL, VAL0, VCUT, VMAX, VSQ
      DOUBLE PRECISION      C, TERM, SUMT
c     ..
c     .. Local Arrays ..
...
c     .. External Functions ..
      ...
      DOUBLE PRECISION      D1MACH, PLKFSAFE
      EXTERNAL  D1MACH, PLKFSAFE
c     ..
c     .. External Subroutines ..
...
c     ..
      ...
      DATA      C2 / 1.438786 / , SIGMA / 5.67032D-8 / , VCUT / 1.5 / ,
...
c ---------------------------------------------------------------------
      DOUBLE PRECISION FUNCTION PLKFSAFE( X )
...
c     ..
c     .. Local Scalars ..
      DOUBLE PRECISION      EX, DEN, XSQ
c     ..
c     .. Parameters ..
      DOUBLE PRECISION      XSMALL
      PARAMETER ( XSMALL = 1.0D-3 )
c     ..
c     .. Intrinsic Functions ..
      INTRINSIC EXP
c     ..
      ...
      IF( X .LT. XSMALL ) THEN
         XSQ = X*X
         PLKFSAFE = XSQ*( 1.0D0 - 0.5D0*X + X*X/12.0D0 )
      ELSE
         EX = EXP( -X )
         DEN = 1.0D0 - EX
...
c     .. Array Arguments ..
      ...
      DOUBLE PRECISION      U0U( MXUMU, * ), UMU( NUMU ), UTAU( NTAU )
c     ..
c     .. Local Scalars ..
...
c     .. Array Arguments ..
      ...
      DOUBLE PRECISION      DTAUC( * ), DTAUCP( * ), FLYR( * ), 
     &          OPRIM( * ), PHI( * ), PMOM( 0:MAXMOM, * ), SSALB( * ), 
...
c ---------------------------------------------------------------------
      DOUBLE PRECISION FUNCTION RATIO( A, B )
...
c     .. Local Scalars ..
      ...
      LOGICAL   PASS1
      DOUBLE PRECISION      ABSA, ABSB, HUGE, POWA, POWB, POWMAX, 
     &                      POWMIN, TINY
c     ..
c     .. External Functions ..
...
c     .. Scalar Arguments ..
     ...
     &          USRTAU, DO_PSEUDO_SPHERE, DELTAMPLUS  
      INTEGER   IBCND, NLYR, NMOM, NPHI, NSTR, NTAU, NUMU
      DOUBLE PRECISION      ACCUR, ALBEDO, BTEMP, DTAUC, FBEAM, FISOT, 
     &          FLUP, PHI, PHI0, RFLDIR, RFLDN, SSALB, TEMIS, TTEMP, 
     &          UMU, UMU0, UTAU, UU, WVNMHI, WVNMLO
c     ..
c     .. Array Arguments ..

      LOGICAL   PRNT( * ), PRNTU0( * )
      DOUBLE PRECISION      PMOM( 0:* ), TEMPER( 0:* )
c     ..
c     .. Local Scalars ..
     ...
     &          USRANS, USRTAS, DO_PSEUDO_SPHERES
      ...
      DOUBLE PRECISION      ACC, ACCURS, ALBEDS, BTEMPS, DTAUCS, ERROR1, 
     &          ERROR2, ERROR3, ERROR4, FBEAMS, FISOTS, PHI0S, PHIS, 
     &          SSALBS, TEMISS, TTEMPS, UMU0S, UMUS, UTAUS, WVNMHS, 
     &          WVNMLS
c     ..
c     .. Local Arrays ..

      LOGICAL   PRNTS( 5 ), PRNU0S( 2 )
      DOUBLE PRECISION      PMOMS( 0:5 ), TEMPES( 0:1 )
c     ..
c     .. External Functions ..
...
c     .. Array Arguments ..
      ...
      INTEGER   IPVT( * ), LAYRU( * )
      DOUBLE PRECISION      ALBMED( * ), AMB( * ), APB( * ), ARRAY( * ), 
     &          CC( * ), CMU( * ), CWT( * ), DFDT( * ), EVAL( * ), 
     &          EVECC( * ), EXPBEA( * ), FLUP( * ), FLYR( * ), GC( * ), 
     &          GL( * ), GU( * ), KK( * ), LL( * ), OPRIM( * ), 
     &          PHASA( * ), PHAST( * ), PHASM( * ), PSI0( * ), 
     &          PSI1( * ), RFLDIR( * ), RFLDN( * ), TAUCPR( * ), 
     &          TRNMED( * ), U0U( * ), UAVG( * ), UTAUPR( * ), UU( * ), 
     &          WK( * ), XR0( * ), XR1( * ), YLM0( * ), YLMC( * ), 
     &          Z( * ), Z0( * ), Z0U( * ), Z1( * ), Z1U( * ), YLMU( * ),
...
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      INTEGER    nlyr
      DOUBLE PRECISION       r0, h_lyr(0:nlyr), h(0:nlyr), umu0
      DOUBLE PRECISION       umu0p(nlyr, nlyr)

c     .. local variable .. 
      INTEGER    lc, n
      DOUBLE PRECISION       cos_theta0, sin_theta0, cos_theta, 
     &                       sin_theta
...
c
c   Called by- DISORT
c   Calls- LEPOLY, ZEROIT, DGBTRF, SOLEIG, TERPEV, SETMTX, SOLVE1,
...
c     .. Array Arguments ..
      ...
      LOGICAL   PRNT( * )
      INTEGER   IPVT( * )
      DOUBLE PRECISION      ALBMED( MAXUMU ), AMB( NN, NN ), 
     &          APB( NN, NN ), ARRAY( NSTR, NSTR ), B( NSTR*NLYR ), 
     &          BDR( NN, 0:NN ), CBAND(9*NN-2,NLYR*NSTR ), 
     &          CC(NSTR, NSTR ), CMU( MXCMU ), CWT( MXCMU ), 
     &          DTAUCP( * ), EVAL( NN ), EVECC(NSTR,NSTR ), 
     &          GC( MXCMU, MXCMU, * ), GL( 0:NSTR, * ), 
     &          GU( MXUMU, MXCMU, * ), KK( MXCMU, * ), LL( MXCMU, * ), 
     &          SQT( * ), TAUCPR( 0:* ), TRNMED( MAXUMU ), 
     &          U0U( MXUMU, * ), UMU( MAXUMU ), WK( MXCMU ), 
     &          YLMC( 0:MXCMU, MXCMU ), YLMU( 0:MXCMU, * ), 
     &          Z(NSTR*NLYR )
...
c     .. Local Scalars ..
      ...
      DOUBLE PRECISION      DELM0, FISOT, SGN, SPHALB, SPHTRN
c     ..
c     .. External Subroutines ..

      EXTERNAL  ALTRIN, ERRMSG, LEPOLY, PRALTR, SETMTX, DGBTRF, SOLEIG,
...
c     .. Dummy sink to silence unused arguments ..
      IF( NUMU.LT.0 ) THEN
         Z(1) = Z(1)
         WK(1) = WK(1)
      END IF
...
c                      ** LU-decompose the coeff. matrix (LAPACK)
      ...
      NCD  = 3*NN - 1
      CALL DGBTRF( NCOL, NCOL, NCD, NCD, CBAND, 9*NN-2, IPVT, INFO )
      IF( INFO .NE. 0 )
     &    CALL ERRMSG('ALBTRN--DGBTRF matrix near singular',.FALSE.)
...
c     .. Array Arguments ..
      ...
      DOUBLE PRECISION      GU( MXUMU, MXCMU, * ), KK( MXCMU, * ), 
     &          LL( MXCMU, * ), TAUCPR( 0:* ), U0U( MXUMU, * ), 
     &          UMU( MAXUMU ), WK( MXCMU )
c     ..
c     .. Local Scalars ..

      INTEGER   IQ, IU, IUMAX, IUMIN, LC, LU
      DOUBLE PRECISION      DENOM, DTAU, EXP1, EXP2, EXPN, MU, PALINT, 
     &                      SGN
c     ..
c     .. Local Arrays ..

      DOUBLE PRECISION      UTAUPR( 2 )
c     ..
c     .. Intrinsic Functions ..
...
c     .. Parameters ..
      ...
      PARAMETER ( DPR = 180.0 / 3.141592653589793 )
c     ..
c     .. Scalar Arguments ..
...
c     .. Array Arguments ..
      ...
      DOUBLE PRECISION      ALBMED( NUMU ), TRNMED( NUMU ), UMU( NUMU )
c     ..
c     .. Local Scalars ..
...
c
c       B        :  Right-hand side vector of Eq. SC(5) going into
c                   DGBTRS; returns as solution vector of Eq.
...
c
c   Called by- ALBTRN
c   Calls- ZEROIT, DGBTRS
c +-------------------------------------------------------------------+

c     .. Scalar Arguments ..

      INTEGER   IHOM, MXCMU, NCOL, NCUT, NN, NLYR, NSTR
      DOUBLE PRECISION      FISOT
c     ..
c     .. Array Arguments ..

      INTEGER   IPVT(NSTR*NLYR )
      DOUBLE PRECISION      B(NSTR*NLYR),CBAND(9*NN-2, NLYR*NSTR), 
     &                      LL( MXCMU, * )
c     ..
c     .. Local Scalars ..
...
c                             ** sources, remainder of B array is zero
   ...
   20    CONTINUE
      ...
      NCD  = 3*NN - 1
      CALL DGBTRS( 'N', NCOL, NCD, NCD, 1, CBAND, 9*NN-2, IPVT,
     &              B, NSTR*NLYR, INFO )
...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      SFLDN, SFLUP
c     ..
c     .. Array Arguments ..

      DOUBLE PRECISION      CMU( MXCMU ), CWT( MXCMU ), 
     &          GC( MXCMU, MXCMU, * ), KK( MXCMU, * ), LL( MXCMU, * ), 
     &          TAUCPR( 0:* )
c     ..
c     .. Local Scalars ..
