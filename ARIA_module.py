...
def get_ri_filepathname(input_string):
    ...
    aria = "/home/g/grainger/ri/"
    file_mapping = {
        "ash"           : aria + "Volcanic_Ash/Eyjafjallajokull/Reed1_2017/eyjafjallajokull-ash_Reed.ri",
        "ice"           : aria + "Water_and_Ice/Ice/200_K_to_300_K/266_K_(Warren_and_Brandt_2008)/ICE_Warren_2008.ri",
        "sulphuric acid": aria + "Acids/Sulphuric/70%_to_79%/75%_300_K_(Palmer_and_Williams_1975)/interpolated/H2SO4_75_Palmer_1975.ri"
    }
...
class RI:
    ...
    expected_header_names = [
        'FORMAT', 'DESCRIPTION', 'DISTRIBUTEDBY', 'SUBSTANCE',
        'SAMPLEFORM', 'TEMPERATURE', 'CONCENTRATION', 'REFERENCE',
        'DOI', 'SOURCE', 'CONTACT', 'COMMENT'
    ]
        ...
        with open(filepathname, 'r') as f:
            ...
            for line in t:
                if line.startswith('#'):
                    header_lines += 1
                    if data_lines > 0:
                        raise ReadError(f"Incorrectly formatted file ({filepathname}): Header not contiguous.")
                else:
                    data_lines += 1
            ...
            for i in range(header_lines):
                ...
                if line[0] != '#':
                    ...
                    if tag_name not in self.expected_header_names:
                        print(f'Unknown header tag "{tag_name}", so ignored (file: {filepathname})')
                        continue
                    try:
            ...
            if 'FORMAT' not in self.header:
                raise ReadError(f"Incorrectly formatted file ({filepathname}): No FORMAT tag in header.")
            ...
            for cl in column_labels:
                if cl not in self.expected_column_names:
                    print(f'Unknown column name "{cl}", so ignored (file: {filepathname})')
                    continue
                self.data[cl] = []
            ...
            if 'wavn' not in self.data:
                self.data['wavn'] = [float(10000) / x if x != 0 else float('nan') for x in self.data['wavl']]
            if 'wavl' not in self.data:
                self.data['wavl'] = [float(10000) / x if x != 0 else float('nan') for x in self.data['wavn']]
            col_lengths = [len(self.data[col]) for col in self.data]
            if len(set(col_lengths)) > 1:
                raise ReadError(f"Incorrectly formatted file ({filepathname}): Data columns have different lengths.")
       ...
       # Determine which data to use
        ...
        if min(wave) < min_x or max(wave) > max_x:
            if out_of_range == 'error':
                ...
                raise ValueError("Invalid value for out_of_range. Use 'error', 'clip', or 'nan'.")
    ...
    def load_refractive_indices(self, composition, wave=None, mode='wavelength', out_of_range='error'):
        """
        ...
        Parameters:
            composition (str): The composition which is
                an ARIA file name
...
def read_ri_file(filepathname, wave=None, mode='wavelength', out_of_range='error'):     
    ...
    Parameters:
        an ARIA    file name
        ARIA_path  path to ARIA data root
        wave (list or array, optional): The target wavelengths or wavenumbers to interpolate to. If None, returns data at full resolution.
        mode (str): 'wavelength' for wave in µm or 'wavenumber' for wave in cm⁻¹.
        out_of_range (str): Behavior for out-of-range values: 'error', 'clip', or 'nan'.
