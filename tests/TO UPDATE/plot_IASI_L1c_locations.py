...
projPC = ccrs.PlateCarree(central_longitude=154)
...
meas_pt_size = (
    iasi_pix * 360 / earth_circumference
)  # sets point size to represent area iasi sees on ground at nadir
ax.scatter(
    l1c.lon, l1c.lat, transform=ccrs.PlateCarree(), c="darkorange", s=meas_pt_size
)  # here the transform argument says which projection are our data in, also PlateCarree equals Cartesian, does not work on zoom
...
ax.add_patch(
    Rectangle(
        (box_lon_min, box_lat_min), 0.6, 0.6, fill=False, transform=ccrs.PlateCarree()
    )
)

# set map extent
extent = plotting.map_extent(
    center_lon=box_lon_min + 0.3, center_lat=box_lat_min + 0.3, lon_span=2, lat_span=2
)

ax.set_extent(extent, crs=ccrs.PlateCarree(central_longitude=box_lon_min + 0.3))
...
gl.xlocator = LongitudeLocator(
    nbins=180
)  # for some reason sets the spacing as 360/nbins, if not specified, creates ticks every 36 deg, which is useless for zoomed plots
