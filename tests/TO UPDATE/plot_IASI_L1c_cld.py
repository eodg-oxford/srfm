...
l1c = oxharp.readers.read_iasi_l1c.Read_Iasi_L1c(
    fl,
    avhrr=avhrr,
    bright=bright,
    chkqal=chkqal,
...
cmap = "viridis"
ax.scatter(
    l1c.lon, l1c.lat, transform=ccrs.PlateCarree(), c=l1c.cld, cmap=cmap, alpha=0.5
)
...
sm.set_array([])
plt.colorbar(sm, ax=ax, label="Cloud cover (%)")

ax.scatter(
    HTHH_lon, HTHH_lat, c="r", s=200, transform=ccrs.PlateCarree()
)  # https://www.fcc.gov/media/radio/dms-decimal

extent = oxharp.plotting.map_extent(
    center_lon=HTHH_lon, center_lat=HTHH_lat, lon_span=50, lat_span=50
)

ax.set_extent(extent, crs=ccrs.PlateCarree(central_longitude=HTHH_lon))
...
ax.set_title("Percentage cloud cover")
