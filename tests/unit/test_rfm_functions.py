...
def test_read_regular_rfm_spectrum(tmp_path):
    """Verify parsing of a regular-grid RFM spectrum.

    Header metadata and spectral values must be preserved.
    ...
    assert [output[f"header{i}"] for i in range(1, 4)] == [
        "header one",
        "header two",
        "header three",
    ]
...
def test_read_irregular_rfm_spectrum(tmp_path):
    """Verify parsing of an irregular-grid RFM spectrum.

    Explicit abscissa values should be returned without regular-grid inference.
...
def test_read_regular_spectrum_validates_point_count(tmp_path):
    """Verify regular spectra reject inconsistent point counts.

    Declared grid metadata must agree with the number of data records.
...
def test_read_profile_sections_and_comma_separators(tmp_path):
    """Verify profile parsing handles sections and comma separators.

    The reader must normalize common native RFM formatting variants.
...
def _driver_input():
    """Build the minimal structured input for RFM driver tests.

    The baseline includes all mandatory sections and one geometry mode.

    Returns:
        Mapping containing mandatory sections and nadir geometry.
    """
    return {
        "HDR": "pytest",
        "FLG": "OPT NAD",
...
def test_construct_driver_table_contents_and_does_not_mutate_input(tmp_path):
    """Verify driver construction writes content without mutating inputs.

    Stable caller mappings are important when generating multiple runs.
...
def test_construct_driver_table_requires_mandatory_and_one_geometry_section(tmp_path):
    """Verify driver construction requires core and geometry sections.

    Missing mandatory records should fail before any native invocation.
...
def test_construct_driver_force_false_preserves_previous_file(tmp_path):
    """Verify non-forced driver writes preserve the previous file.

    Existing content is moved aside rather than silently overwritten.
...
def test_construct_levels_file_formats_and_preserves_old_copy(tmp_path):
    """Verify level files are formatted and old copies preserved.

    Line wrapping and backup behavior are checked together.

    Args:
        tmp_path: Pytest temporary-path fixture.
    """
    rfm_functions.construct_rfm_output_levels_file([2, 0, 1], tmp_path)
    assert (tmp_path / "alts.lev").read_text(
        encoding="utf-8"
...
def test_construct_irregular_grid_file(tmp_path):
    """Verify irregular RFM grid files contain explicit samples.

    Every requested wavenumber must appear in the generated file.

    Args:
        tmp_path: Pytest temporary-path fixture.
    """
    relative = rfm_functions.construct_rfm_grid_file(
        [1000, 1000.5, 1002], rfm_fldr=tmp_path
    )
    assert relative == "./rfm_files/grid.spc"
    lines = (
        (tmp_path / "rfm_files" / "grid.spc").read_text(encoding="utf-8").splitlines()
    )
...
def test_atmosphere_write_read_round_trip(tiny_atmosphere):
    """Verify atmosphere data survive an RFM-format round trip.

    Profile names, units, coordinates, and values should be retained.
...
def test_write_atmosphere_validates_height_and_lengths(tmp_path):
    """Verify atmosphere writing validates heights and profile lengths.

    Inconsistent vectors cannot form a valid column-oriented atmosphere.
    ...
    with pytest.raises(ValueError, match="expected 2"):
        rfm_functions.write_atm_file(
            {"HGT [km]": [0, 1], "TEM [K]": [280]}, tmp_path / "x.atm"
        )


def test_write_xsc_uses_hitran_fixed_width_format(tiny_xsc_file):
    """Verify cross sections use the HITRAN fixed-width format.

    Header width and spectral records must remain native-reader compatible.

    Args:
        tiny_xsc_file: Synthetic cross-section fixture.
    """
    ...
    np.testing.assert_allclose(
        [float(value) for value in lines[1].split()], [1e-20, 2e-20, 1e-20]
    )


def test_write_xsc_validates_required_fields_and_lengths(tmp_path):
    """Verify cross-section writing validates fields and lengths.

    Missing metadata and inconsistent coefficient counts are rejected.
