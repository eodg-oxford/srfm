...
def _valid_integrity_input():
    return dict(maxmom=4, maxcmu=4, maxumu=1, maxphi=1, ibcnd=0, onlyfl=False, dtauc=np.array([0.1, 0.2]), ssalb=np.array([0.5, 1.0]), temper=np.array([280, 270, 260]), wvnmlo=999, wvnmhi=1001, utau=np.array([0.3]), umu0=0.5, phi0=0, umu=np.array([0.8]), phi=np.array([0]), btemp=290, ttemp=250, temis=1)


def test_disort_integrity_accepts_physically_valid_small_case():
    assert disort_functions.test_disort_input_integrity(**_valid_integrity_input()) is True


@pytest.mark.parametrize(
    ("field", "value", "message"),
    [("maxmom", 2, "Maxmom"), ("maxcmu", 3, "even"), ("ssalb", np.array([1.1]), "ssalb"), ("wvnmlo", 1002, "Wvnmlo"), ("umu0", 2, "Umu0"), ("umu", np.array([0.8, 0.2]), "increasing"), ("phi", np.array([361]), "between")],
)
