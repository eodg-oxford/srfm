...
def test_legendre_expansion_of_constant_phase_function():
    ...
    coefficients, used = optical.legendre_polynomial_expansion(32, nodes, weights, np.ones(32))
...
def test_normalised_and_regular_legendre_coefficients_have_expected_scaling():
    ...
    normalised, _ = optical.normalised_legendre_polynomial_expansion(32, nodes, weights, phase)
...
def test_phase_reconstruction_from_both_coefficient_conventions():
    ...
    normalised = optical.phase_from_normalised_legendre(3, [1, 0.2, 0.05], len(points), points)
...
def test_user_supplied_refractive_index_scalar_and_array():
    scalar = optical.get_ri("ri", refractive_index=1.5 - 0.01j, wave=[1, 2], wave_size=2)
    array = optical.get_ri("ri", refractive_index=[1.5 - 0.01j, 1.6 - 0.02j], wave=[1, 2], wave_size=2)
