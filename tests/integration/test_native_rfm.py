...
def test_structured_driver_runs_real_rfm_and_captures_optical_depths(
    ...
    result = rfm_helper.run_rfm_with_parameters(
        ...
        gases=("F11",), atmosphere=(tiny_altitude_grid, tiny_atmosphere),
        tangent=("1.0",), lev=("0", "1", "2"), sfc=("TEMSFC=290",),
        xsc=(tiny_xsc_file,), run_id="pytest", directory=tmp_path,
        output_mode="capture", enable_capture=True, optical_levels=(0, 1, 2),
    )
    assert result.ok
